...
every tracked path under an explicit source-layout policy, select an explicit
root `README.md` or `TOC.md`, validate the two known input-section labels
without weakening their exact two-IPSW contract, and emit a canonical census
in which every selected payload is either structurally migratable or blocked
...
| Versions, builds, and input artifacts | Exact selected source report structure and content |
...
| Transformation | Root README/TOC and known heading variants normalize to the same exact two-IPSW record | Unresolved |
...
diff, and a report title does not establish platform or destination routing.
Absence of both reviewed root entrypoints cannot be repaired from the path. A
redirect, OTA/RSR comparison, or non-IPSW input is a blocked row until a later
schema explicitly models it. A successful parse proves only census eligibility,
not migration readiness.

## Stop conditions

Do not call the census closed if any tracked file is unclassified or classified
twice, an exclusion overlaps a payload, a policy root is absent, either root
entrypoint or known input label can bypass the exact two-IPSW check, malformed
or missing metadata is silently inferred, output depends on worktree state, or
the source worktree changes.
...
| Transformation | One parser enforces the same exact title and two-IPSW contract for 104 README `## IPSWs`, 37 README `## Inputs`, and 12 root TOC `## IPSWs` rows; all other structures are blocked | Closed |
...
| --- | ---: |
| Payload roots | 156 |
| Ordinary two-IPSW payloads | 153 |
| Blocked payloads | 3 |
| Root TOC ordinary payloads | 12 |
| Redirect blockers | 1 |
| Non-IPSW input blockers | 1 |
| Unsupported report blockers | 1 |
...
`4865c3238ea8adce4c131a4c28c893d55ef0d61f4172527dc4411d6fb3ec78d1`.
...
- Report tests accept both reviewed root entrypoints and both known ordinary
  headings, preserve `TOC.md` without synthesizing a README, and reject nested
  or arbitrary entrypoints, mixed or duplicate headings, extra section content,
  title/input disagreement, and non-IPSW input artifacts.
- Atomic generation writes a normal non-executable `0644` output file.
- Formatting, lint, type checking, and all 127 tests passed.

## Bounded conclusion

The scoped census is closed as a deterministic migration-planning input. It
accounts for every tracked file at the frozen legacy commit and identifies 153
structurally ordinary payloads plus 3 explicit blockers. It does not establish
